            ...
            <table width="626" height="100%" border="0" cellpadding="0" cellspacing="0">
              <tr>
                <td height="257" align="center" valign="top" background="images/image_08.gif"><table width="600" height="271"  border="0" cellpadding="0" cellspacing="0">
                  <tr>
                    <td height="22" align="center" valign="top" class="word_orange"><strong>查询书本信息</strong></td>
                  </tr>
                  <tr>
...
<input type="submit" name="Submit" value="搜索" onClick="return check(form)">
                    ...
                    /*  当前页提交的搜索 $page初始化为1  读入字符*/ 
                    if($_POST[Submit]!=""){
                    $page=1;
                    $keyword=$_POST['txt_keyword'];
                    }else if($_SESSION['keyword']!=""){//如果没提交搜索 读入session
                    ...
                    for($i=0; $i<$num; $i++)
                    {
                       ...
                       $cnt ++;
                        if($cnt == 1)
                      $info .= "where bookname like '%$name_arr[$i]%' ";
                      else
                  ...
                  $sql=mysql_query("select * from books  $info");
                  $message_count=mysql_num_rows($sql);      
                  $page_count=ceil($message_count/$page_size);      //根据记录总数除以每页显示的记录数求出所分的页数
                  $offset=($page-1)*$page_size;           //计算下一页从第几条数据开始循环  
                  $sql=mysql_query("select * from books $info limit $offset,$page_size");
                      ...
                      <tr bgcolor="#FFFFFF">
                        ...
                        <td><?php echo mb_substr($row->content,0,20,"UTF-8");
                        if(strlen($row->content)>30)
                        echo "...";
                         ?></td>
                          
                           <td align="center"><a href="check_comment.php?id=<?php echo $row->bookid;?>"><img src="images/update.gif" width="22" height="22" border="0"></a></td>                      </tr>
                    <?php
                      ...
                      <table width="550" border="0" cellspacing="0" cellpadding="0">
                        <tr>
                          <!--  翻页条 -->
                            ...
                            if($page!=1){
                            /*  显示“首页”超链接  */
                            echo  "<a href=search_book.php?page=1>首页</a>&nbsp;";
                            /*  显示“上一页”超链接  */
                            echo "<a href=search_book.php?page=".($page-1).">上一页</a>&nbsp;";
                            }
                            /*  如果当前页不是尾页  */
                            if($page<$page_count){
                            /*  显示“下一页”超链接  */
                            echo "<a href=search_book.php?page=".($page+1).">下一页</a>&nbsp;";
                            /*  显示“尾页”超链接  */
                            echo  "<a href=search_book.php?page=".$page_count.">尾页</a>";
                            }
