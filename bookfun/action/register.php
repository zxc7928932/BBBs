<?php
	...
	$row=mysql_query("select * from user where username='$user_name'");		//此用户名的数据
	...
	/*********************给密码加密******************/	
	$sql=mysql_query("insert into user(username,pwd)values('$user_name','$password')");
	//插入数据
	if (!$sql){		//插入失败，跳转回注册页面
     echo "<script> alert('系统繁忙，请重试!');window.location.href='../index.php';</script>";
}
	else{			//注册成功，转到主页面
	$row=mysql_query("select * from user where username='$user_name'");
