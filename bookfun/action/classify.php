...
<body>
  			...
  			<div class="col-sm-6 col-md-4" id="book_form"  style="margin-left:80px">
  				<div class="thumbnail" >
  					...
  					<div class="caption">
  						...
  						<p>
  							<a href="show.php?id=<?php $row->id;?>" class="btn btn-primary" role="button">详情</a> 
  							 <?php if($num==0){ ?>
    						<a href="collection.php?id=<?php echo $row->id;?>" class="btn btn-default" role="button">收藏</a>
    						<?php }
   							 else { ?>
   							 <a href="delete_collection.php?id=<?php echo $row->id;?>" class="btn btn-default" role="button">取消收藏</a>
    						<?php } ?>
  						</p>
  					</div>
  				</div>
  ...
  if($page!=1){     
  	/*  显示“首页”超链接  */
  	echo  "<a href=classify.php?page=1>首页</a>&nbsp;";
  	
  	if($page>1){
  		/*  显示“上一页”超链接  */
  		echo "<a href=classify.php?page=".($page-1).">上一页</a>&nbsp;";
  	}
  }
  /*  如果当前页不是尾页  */
  if($page<$count){           
  	...
  	/*  显示“下一页”超链接  */
  	echo "<a href=classify.php?page=".($page+1).">下一页</a>&nbsp;";
  	
  	/*  显示“尾页”超链接  */
  	echo  "<a href=classify.php?page=".$page_count.">尾页</a>";
  }
