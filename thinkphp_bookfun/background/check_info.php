				...
				$id = $_GET['id'];
					...
					if ($page==""){
					   ...
					   if (is_numeric($page)){
						...
						$sql=mysql_query("select * from comment where uid =$id");//查找当前用户的评论
						$row=mysql_fetch_object($sql);
						$message_count=mysql_num_rows($sql);
						$page_count=ceil($message_count/$page_size);	  	//根据记录总数除以每页显示的记录数求出所分的页数
						$offset=($page-1)*$page_size;						//计算下一页从第几条数据开始循环  
						$sql=mysql_query("select * from comment where uid=$id limit $offset, $page_size");//搜索当前页信息
						$row=mysql_fetch_object($sql);					//得到对象
						if(!$row){		//若为空
							echo "<font color='red'>用户暂无评论!</font>";
						}
						do{
						...
						$sql1=mysql_query("select * from books where bookid =$book");//得到该评论用户信息
                      ...
                      <tr bgcolor="#FFFFFF">
                        ...
                        echo "...";
                         ?></td>
                           <td align="center"><a href="check_com_ok.php?id=<?php echo $row->commentid;?>"><img src="images/delete.gif" width="22" height="22" border="0"></a></td>
						 </tr>
					<?php
                      ...
                      <table width="550" border="0" cellspacing="0" cellpadding="0">
                        <tr>
                          <!--  翻页条 -->
							...
							if($page!=1){
							/*  显示“首页”超链接  */
							echo  "<a href=check_info.php?page=1&id=".$id.">首页</a>&nbsp;";
							/*  显示“上一页”超链接  */
							echo "<a href=check_info.php?page=".($page-1)."&id=".$id.">上一页</a>&nbsp;";
							}
							/*  如果当前页不是尾页  */
							if($page<$page_count){
							/*  显示“下一页”超链接  */
							echo "<a href=check_info.php?page=".($page+1)."&id=".$id.">下一页</a>&nbsp;";
							/*  显示“尾页”超链接  */
							echo  "<a href=check_info.php?page=".$page_count."&id=".$id.">尾页</a>";
							}
